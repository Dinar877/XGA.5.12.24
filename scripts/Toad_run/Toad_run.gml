function Toad_run() {
	...
	if (inst2 > -1)
	{
		...
		{
			if (sprite_index = spr_enemy_toad_dash_left) && ((global.invisibility = 1) or (inst2.inst_collision <= 0) or (position_meeting(inst2.x+(facing*8),inst2.y,obj_block)) or (inst2.inst_collision2 > 0) or (inst2.inst_blockfree > 0))
				{
					sprite_index = spr_enemy_toad_standleft;
					State_machine_switch_state(Toad_stand);
					exit;
				}
			else if (sprite_index = spr_enemy_toad_dash_right) && ((global.invisibility = 1) or (inst2.inst_collision <= 0) or (position_meeting(inst2.x+(facing*8),inst2.y,obj_block)) or (inst2.inst_collision2 > 0) or (inst2.inst_blockfree > 0))
				{
