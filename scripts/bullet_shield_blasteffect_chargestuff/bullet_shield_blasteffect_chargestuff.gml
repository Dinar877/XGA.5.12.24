function bullet_shield_blasteffect_chargestuff() {
	  with(obj_blast_effect)
	 {
	 if (global.shoot = 1) && (global.beam_shield_available = 1)
	 {
	...
	if (sprite_index = spr_SHIELD_charging_begin) && (started_charging = 1) && (timer_charging < charge_time-global.charge_booster)
		{
		    timer_charging += 1/60;
		}
	if (sprite_index = spr_SHIELD_charging_begin) && (image_index == image_number - 1) && (started_charging = 1) && (timer_charging >= charge_time-global.charge_booster)
		{
		    fully_charged = 1;
		}
	 }
