 /// @description Insert description here
// You can write your code in this editor

State_machine_create();
State_machine_switch_state(Tentacely_idle)
go = 0;
facing = -1;
turning = 0;
snd = -1
sword_spriteindex = 0

random_pickup1 = irandom_range(0,5);
random_pickup2 = irandom_range(0,5);
random_pickup3 = irandom_range(0,5);

prep_now = 0;
knockback_now = 0;
run_now = 0;

//attack variables
attackclosego = (collision_line(id.x,id.y,id.x+(facing*30),id.y,obj_player,false,true))
attackrangego = (attackclosego <= 0) && (collision_line(id.x,id.y,id.x+(facing*100),id.y,obj_player,false,true))

inst1 = 0;
inst2 = 0;
inst4 = 0;
inst2 = instance_place(x,y,obj_enemy_tentacely_hitbox);
inst_self = self;
inst_player = -1;

set_direction = 0;
set_dir2 = 0;

FadeNow = 0;
started1 = 0;
started2 = 0;

started_idle = 0;
idleY = 0;
idleY2 = 0;
idleswitch1 = 1;
idleswitch2 = 0;
timer = 0;
timer2shoot = 1;
timer2burst = 0
inst1created = 0;

fade_color = shader_get_uniform(shd_flash_fade_white, "fade_color");
NewColor = shader_get_uniform(shd_flash_fade_white, "NewColor");

//for getting hurt by player
stop1 = 0;
enemy_hurt = 0;
enemy_almost_hurt = 0;
stay = 0;


go = 0;
facing = -1;
turning = 0;
sword_spriteindex = 0

random_pickup1 = irandom_range(0,5);
random_pickup2 = irandom_range(0,5);
random_pickup3 = irandom_range(0,5);


//idle animations and state stuff
idleanim_walktimer = 0
idleanim_turntimer = 0

inst_circle = -1
inst_collision = -1
inst_collision2 = -1

prep_now = 0;
knockback_now = 0;
run_now = 0;
inst4 = 0;
inst_self = self;

set_direction = 0;
set_dir2 = 0;

FadeNow = 0;
started1 = 0;
started2 = 0;

started_idle = 0;

fade_color = shader_get_uniform(shd_flash_fade_white, "fade_color");
NewColor = shader_get_uniform(shd_flash_fade_white, "NewColor");

//for getting hurt by player
stop1 = 0;
enemy_hurt = 0;
enemy_almost_hurt = 0;
stay = 0;


if (global.mode_casual == 0)
{
	hp = 30;
}
else hp = 30;

sword_spriteindex = 0
sword_deadswitch = 0

knockback = 0;
metal = 0