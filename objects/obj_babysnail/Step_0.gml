...
if (sprite_index = spr_babysnail_left) && (obj_player.x > self.x)
{
	sprite_index = spr_babysnail_tl2r;	
}
else if (sprite_index = spr_babysnail_tl2r) && (image_index >= image_number-1)
{
	sprite_index = spr_babysnail_right;	
}

if (sprite_index = spr_babysnail_right) && (obj_player.x < self.x)
{
	sprite_index = spr_babysnail_tr2l;	
}
else if (sprite_index = spr_babysnail_tr2l) && (image_index >= image_number-1)
{
	sprite_index = spr_babysnail_left;	
}
...
if (place_meeting(x,y,obj_player)) && (global.upgrade_process = 0)
{
	...
	with(tex2)
	{
		text = " COLLECT ENOUGH OF THESE AND RETURN THEM TO THEIR MOTHER FOR REWARDS.";
		spd = 1;
		spawnerID = other.id;
		font = font_regular1;
