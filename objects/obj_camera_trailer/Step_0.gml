...
or (!instance_exists(obj_player_trailer))
{
	exit;	
}
...
&& (global.climbing == 0) && (instance_exists(obj_mapchecker2)) && (instance_exists(obj_camera_stopperU)) && (instance_exists(obj_camera_stopperD))
{
	...
	{
		y = obj_player_trailer.y-yshake;
	}
	
	yshake = lerp(yshake,-1,0.15);
	
	distance_to_border3 = obj_camera_trailer.y-obj_camera_stopperU.y-90;
	distance_to_border4 = obj_camera_trailer.y-obj_camera_stopperD.y+90;
...
//room border
if (instance_exists(obj_player_trailer))
{
	//camera shift down, starting room transition
	if (obj_player_trailer.y != y) && ((global.mapgrid_yZeroMove = 1) or (global.mapgrid_1x1 = 1)) && (global.room_transition_prep == 1)
	...
	&& (global.camera_shiftdown = 1) && (global.stopper_2ndscrnshot == 0)
	{
		...
		if (obj_player_trailer.y > y)
		{
			y = lerp(y,obj_player_trailer.y+2,0.2)
			with(obj_scrn_roomtransition)
			{
				y = obj_camera_trailer.y-90;
			}
		}
		else if (obj_player_trailer.y < y)
		{
			y = lerp(y,obj_player_trailer.y-2,0.2)
			with(obj_scrn_roomtransition)
			{
	...
	&& (global.camera_shiftdown = 1) && (global.stopper_2ndscrnshot == 0)
	{
		...
		with(obj_scrn_roomtransition)
			{
				y = round(y)
				var ybob = y-obj_camera_trailer.y+90
				var ybob2 = 0
			}
		global.ygridend = 0
		global.ygrid = 0
	...
	{
		collision_L = collision_rectangle(x+170,y+100,x-170,y-100,obj_camera_stopperL,false,true);
		collision_R = collision_rectangle(x+170,y+100,x-170,y-100,obj_camera_stopperR,false,true);
		collision_U = collision_rectangle(x+170,y+100,x-170,y-100,obj_camera_stopperU,false,true);
		collision_D = collision_rectangle(x+170,y+100,x-170,y-100,obj_camera_stopperD,false,true);
		...
		if (instance_exists(obj_camera_stopperL)) && (instance_exists(obj_camera_stopperR)) && (global.mapgrid_1x1 = 0) && (global.mapgrid_xZeroMove = 0)
		{
			...
			with(obj_camera_trailer)
			{
				if !(global.climbing)
				{
					x = obj_player_trailer.x;
				}
				
				distance_to_border = obj_camera_trailer.x-obj_camera_stopperL.x-160;
				distance_to_border2 = obj_camera_trailer.x-obj_camera_stopperR.x+159;
		...
		&& (global.room_transition_prep == 0)
		{
			...
			with(obj_camera_trailer)
			{
				...
				{
					y = obj_player_trailer.y-yspin-yshake-yshake2;
				}
				global.ygrid = 0
				distance_to_border3 = obj_camera_trailer.y-obj_camera_stopperU.y-90;
				distance_to_border4 = obj_camera_trailer.y-obj_camera_stopperD.y+90;
