...
if (((stringy == " ") && (time2 > 0) && (floor(time)+time2 > 46)) or (stringy == "\n")) && (switch1 = 0) && (floor(time)+message_pos+time2 < text_length)
{
		...
		if (id.y < 130)
		{
			...
			{
				tex = instance_create_depth(x,y+12+(12*string_positionY),id.depth-5,obj_texty_intro_xga);
			}
			else if (string_positionY == 0)
			{
				tex = instance_create_depth(x,y+12,id.depth-5,obj_texty_intro_xga);
			}
		...
		else if (id.y >= 130)
		{
			...
			instance_create_depth(x+283,y+15,id.depth-5,obj_text_spawning_arrow)
			with(obj_text_spawning_arrow)
			{
				textyID = other.object_index
				spawnerID = other.spawnerID
				message_pos = other.time
				//string_positionY = other.string_positionY
			}
		}
}
else if (((time2 > 0) && (floor(time)+time2 > 46)) or (stringy == "\n")) && (switch1 = 0) && (floor(time)+message_pos+time2 >= text_length)
{
	with(spawnerID)
	{
		time = (other.time+other.message_pos)
