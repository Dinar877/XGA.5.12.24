...
if (pauser <= 0)
	...
	if (pauser > 0) && (!instance_exists(obj_text_spawning_arrow))
	{
		print = string_copy(text,0+(message_pos),time)
		draw_text_ext_color(x,y,print,11,286,c_white,c_white,c_gray,c_gray,image_alpha)
		instance_create_depth(x+278,y+5,obj_scrnDark_navigational2.depth-5,obj_text_spawning_arrow)
	}
	...
	if (pauser_end > 0) && (!instance_exists(obj_text_spawning_arrow_ender_biome1_1))
	{
		print = string_copy(text,0+(message_pos),time)
		draw_text_ext_color(x,y,print,11,286,c_white,c_white,c_gray,c_gray,image_alpha)
	}
