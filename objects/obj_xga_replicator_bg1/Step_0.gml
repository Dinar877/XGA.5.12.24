...
if (global.cutscene_trueintro_sparkexchange = 1)
{
	timer += (1/room_speed)
}
...
if (on = 3)
{
	audio_sound_gain(snd_alarm,0,3000)	
	audio_sound_gain(snd_alarm2,0,3000)	
...
{
	var sndy1 = audio_play_sound(snd_beamexplosion1,1000,false,global.sfx_volume)	
	audio_sound_gain(sndy1,switch1,0)
	audio_sound_gain(sndy1,0,1000)
	switch1 -= (1/5)
}
else if (on = 6) && (!audio_is_playing(snd_beamexplosion1)) && (switch1 <= 0)
{
	...
	room_goto(rm_trueintro_3_shattered)
	audio_stop_sound(snd_heartbeat)
	global.cutscene = 1
