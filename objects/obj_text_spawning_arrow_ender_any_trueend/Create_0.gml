/// @description Insert description here
// You can write your code in this editor
counter1 = 0;
spawnerID = -1;
markerX = 0;
markerY = 0;
markerID = -1;

if (instance_exists(obj_texty_skip))
{
	instance_destroy(obj_texty_skip)
}

if (instance_exists(obj_texty_generic))
{
	instance_destroy(obj_texty_generic)
	textyID = obj_texty_generic
}

if (instance_exists(obj_texty_generic_biome_1_2))
{
	instance_destroy(obj_texty_generic_biome_1_2)
	textyID = obj_texty_generic_biome_1_2
}

if (instance_exists(obj_texty_TRUEgeneric))
{
	instance_destroy(obj_texty_TRUEgeneric)
	textyID = obj_texty_TRUEgeneric
}

if (instance_exists(obj_texty_TRUEgeneric_nonmarker))
{
	instance_destroy(obj_texty_TRUEgeneric_nonmarker)
	textyID = obj_texty_TRUEgeneric_nonmarker
}

if (instance_exists(obj_texty_marker))
{
	instance_destroy(obj_texty_marker)
	textyID = obj_texty_marker
}








if (instance_exists(obj_text_spawning_0_1))
{
	spawnerID = obj_text_spawning_0_1
}

if (instance_exists(obj_text_spawning_any))
{
	spawnerID = obj_text_spawning_any
}

if (instance_exists(obj_text_spawning_teleport))
{
	spawnerID = obj_text_spawning_teleport
}



if (instance_exists(obj_text_spawning_biome1_1))
{
	spawnerID = obj_text_spawning_biome1_1
}

if (instance_exists(obj_text_spawning_biome1_2))
{
	spawnerID = obj_text_spawning_biome1_2
}

if (instance_exists(obj_text_spawning_S2_1))
{
	spawnerID = obj_text_spawning_S2_1
}

if (instance_exists(obj_text_spawning_S3_1))
{
	spawnerID = obj_text_spawning_S3_1
}

if (instance_exists(obj_text_spawning_S4_1))
{
	spawnerID = obj_text_spawning_S4_1
}

//gamepad variable stuff
gamepad_counter = 0;
gamepad_counter2 = 0;
correct_slot = 0;

right_key = 0
right_key_pressed = 0

left_key = 0
left_key_pressed = 0

up_key = 0
up_key_pressed = 0

down_key = 0
down_key_pressed = 0


jump = 0
dash = 0
shoot = 0
shoot_pressed = 0

Select = 0;

surface = -1;
surface2 = -1;
surface3 = -1;
surface2_alpha = 0;
surface2pos = 1;
surface2neg = 0;
navigation_effect_index = 0;


