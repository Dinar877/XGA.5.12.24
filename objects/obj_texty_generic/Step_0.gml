...
if (((stringy == " ") && (time2 > 0) && (floor(time)+time2 > 46)) or (stringy == "\n")) && (switch1 = 0)
{
		...
		if (instance_number(obj_texty_generic) <= 2)
		{
			spd = 0;
			tex = instance_create_depth(x,y+12,id.depth-5,obj_texty_generic);
		...
		else if (instance_number(obj_texty_generic) > 2) && (spd > 0)
		{
			pauser = 1;
			spd = 0;
			with(spawnerID)
...
&& (!instance_exists(obj_text_spawning_arrow_preender_any_any))
{
	instance_create_depth(x+278,y+5,id.depth-5,obj_text_spawning_arrow_preender_any_any)
}
