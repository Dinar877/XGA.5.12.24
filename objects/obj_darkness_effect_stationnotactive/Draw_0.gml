...
{
	light_surface2 = surface_create(660, 280);
}
...
if (surface_exists(light_surface2))
{
	...
	draw_sprite_ext(spr_light_player_small,0, 330, 140,scale1,scale1,random_range(0,360),c_white,1);
	...
	{
		draw_surface(light_surface2, obj_player.x-330+xloc, obj_player.y-140+yloc)
	}
	else if (global.climbing == 0) && (instance_exists(obj_trueopening_player_wakeup))
	{
		draw_surface(light_surface2, obj_trueopening_player_wakeup.x-330, obj_trueopening_player_wakeup.y-140)
	}
	else if (global.climbing == 0) && (!instance_exists(obj_trueopening_player_wakeup)) && (!instance_exists(obj_player))
	{
		draw_surface(light_surface2, x-330, y-140)
	}
