...
if (hp > 0) && (instance_exists(obj_player)) && (instance_exists(inst2))
{
	...
	with(inst2)
	{
		//checks for player - moves closer
		inst_circle = collision_circle(x,y,130,obj_player,false,true)
		
		//checks for player - aiming path exactly right
		inst_circle2 = (place_meeting(x+(facing*90),y,obj_player));
		...
		collision_othertoad = (place_meeting(id.x,id.y,obj_hitbox_hellcannon_right))
	}
