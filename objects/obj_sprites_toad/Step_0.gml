...
{
	xmin = inst2.x;
	xmax = xmin + (sprite_get_width(spr_enemy_toad_collisionbox)/2);
	ymin = inst2.y;
	ymax = ymin + (sprite_get_height(spr_enemy_toad_collisionbox)/2);
}
...
if (hp <= 0)
{
	...
	with(instance_place(x,y,obj_detectionbox_toad))
	{
		instance_destroy();
	}
	with(inst2)
...
if (hp > 0) && (instance_exists(obj_player)) && (instance_exists(inst2))
{
	...
	{
		//checks for player
		inst_circle = collision_circle(x,y,100,obj_player,false,true)
		...
		collision_othertoad = (place_meeting(id.x,id.y,obj_hitbox_toad))
	}
